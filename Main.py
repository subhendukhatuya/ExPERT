...
def train_epoch(model, training_data, optimizer, pred_loss_func, opt):
    ...
    for batch in tqdm(training_data, mininterval=2,
        ...
        """ prepare data """
        event_time, time_gap, event_type, user_emb  = map(lambda x: x.to(opt.device), batch)


        """ forward """
        optimizer.zero_grad()

        enc_out, prediction = model(event_type, event_time, user_emb)
        ...
        # type prediction
        pred_loss, pred_num_event = Utils.type_loss(prediction[0], event_type, pred_loss_func, model.num_types)
        ...
        # uncomment following line for rmse 
        ...
        total_num_ex = 0
        for event_type_ex in Constants.event_types_moodle_ex:
            total_num_ex += (torch.sum(event_type[:,1:].eq(event_type_ex).type(torch.long))).item()
        
        total_num_event += event_type.ne(Constants.PAD).sum().item() - total_num_ex
        # we do not predict the first event
        total_num_pred += event_type.ne(Constants.PAD).sum().item() - event_time.shape[0] - total_num_ex
...
def eval_epoch(model, validation_data, pred_loss_func, opt):
    ...
    with torch.no_grad():
        for batch in tqdm(validation_data, mininterval=2,
            ...
            event_ll, non_event_ll = Utils.log_likelihood(model, enc_out, event_time, event_type)
            event_loss = -torch.sum(event_ll - non_event_ll)
            splitted_truth_type, _, pred_num = Utils.splitted_type_loss(prediction[0], event_type, pred_loss_func, model.num_types)
            #_, pred_num = Utils.type_loss(prediction[0], event_type, pred_loss_func)


            test_event, test_prediction = Utils.split_tensor(prediction[1], event_time)
            se = Utils.time_loss(test_prediction, test_event,splitted_truth_type)
            ...
            total_num_ex = 0 
            for event_type_ex in Constants.event_types_moodle_ex:
                total_num_ex += torch.sum(splitted_truth_type[:,1:].eq(event_type_ex).type(torch.long)).item()
            
            total_num_event += test_event.ne(Constants.PAD).sum().item() - total_num_ex
            # we do not predict the first event
            total_num_pred += test_event.ne(Constants.PAD).sum().item() - event_time.shape[0] - total_num_ex
